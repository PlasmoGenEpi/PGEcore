...
opts <- list(
  ...
  make_option(
    "--coi_range",
    help = str_c("Single COI value (e.g. '2') or comma-separated values (e.g. '1,2,3') for the COI range. ",
                 "This should be the largest COI observed in the data or if length is greater than 1, a zero truncated Poisson distribution on COI is assumed",
                 "Default is COI range of 1:3"),
    type = "character",
    default = "1, 2, 3"
  ),
  make_option(
...
coi_range <- as.numeric(strsplit(arg$coi_range, ",", fixed = TRUE)[[1]] |> trimws())
if (anyNA(coi_range)) stop("Could not parse --coi_range; use a comma-separated list like \"1,2,3\"")
...
#' @param coi_range An integer or numeric vector of possible COI (complexity of infection) range. If length > 1, the estimation will assume a zero-truncated Poisson distribution on COI. Otherwise, the estimation will assume COI is fixed.
