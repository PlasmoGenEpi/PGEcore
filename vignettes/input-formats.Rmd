...
from one step can be inputs to another. Bundled examples live in
`inst/extdata/` after you install the package (or in that folder in the source
tree).
...
wrappers (`dcifer_*`, `FreqEstimationModel_wrapper`, …)
...
Rscript exec/coiaf_wrapper \
  --snp_data my_snp_calls.tsv \
  --output coi_estimates.tsv
```

## Allele / microhaplotype table

**Columns:** `specimen_name`, `target_name`, `seq`, `reads`

**Example:** `example_allele_table.tsv`  
**Used by:** `estimate_coi_naive`, `moire_wrapper`, `malariaem_wrapper`,
`allele_per_locus_summary`, `per_locus_popgen_summary`, many allele-based
wrappers
...
Rscript exec/estimate_coi_naive \
  --input_path my_allele_table.tsv \
...
**Example:** `example_amino_acid_calls.tsv`  
...
```{r}
readr::read_tsv(
  file.path(extdata, "example_amino_acid_calls.tsv"),
  show_col_types = FALSE,
  n_max = 3
)
```
